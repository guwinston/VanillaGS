from typing import NamedTuple
import torch.nn as nn
import torch
import os
import json
import math
import numpy as np
from tqdm import tqdm
from plyfile import PlyData
from matplotlib import pyplot as plt
from PIL import Image
import time
from vanilla_batch_gaussian_splatting import batch_rasterize_gaussians

class BatchCamera:
    def __init__(self, cameras_json, device):
        self.id = []
        self.img_name = []
        positions = []
        rotations = []
        view_mats = []
        proj_mats = []
        tan_fovxs = []
        tan_fovys = []
        for camera_json in cameras_json:
            self.id.append(camera_json['id'])
            self.img_name.append(camera_json['img_name'])
            self.width = camera_json['width'] // 2
            self.height = camera_json['height'] // 2
            self.zFar = 1000.0
            self.zNear = 0.01
            self.device = device

            positions.append(torch.tensor(camera_json['position']))
            rotations.append(torch.tensor(camera_json['rotation']))
            view_mats.append(self.get_viewmatrix(camera_json['rotation'],camera_json['position']))
            proj_mats.append(self.get_projmatrix(self.zNear, self.zFar, camera_json['fx'], camera_json['fy'], 
                                                 self.width, self.height, 
                                                 camera_json['rotation'], camera_json['position']))
            tan_fovxs.append(self.get_tanfov(self.width,  camera_json['fx']))
            tan_fovys.append(self.get_tanfov(self.height, camera_json['fy']))
        self.position = torch.stack(positions, dim=0).reshape(-1).to(self.device)
        self.rotation = torch.stack(rotations, dim=0).reshape(-1).to(self.device)
        self.view_mat = torch.stack(view_mats, dim=0).reshape(-1).to(self.device)
        self.proj_mat = torch.stack(proj_mats, dim=0).reshape(-1).to(self.device)
        self.tan_fovx = torch.tensor(tan_fovxs, dtype=torch.float).to(self.device)
        self.tan_fovy = torch.tensor(tan_fovys, dtype=torch.float).to(self.device)

    def get_tanfov(self, pixels, focal):
        return pixels / (2 * focal)

    def get_viewmatrix(self, rotation, position):
        c2w = torch.eye(4)
        c2w[:3, :3] = torch.tensor(rotation, dtype=torch.float)
        c2w[:3, 3] = torch.tensor(position, dtype=torch.float)
        w2c = torch.inverse(c2w)
        return w2c.transpose(0, 1).to(self.device)
    
    def get_projmatrix(self, zNear, zFar, fx, fy, w, h, rotation, position):
        def getProjectionMatrix(znear, zfar, fovX, fovY):
            tanHalfFovY = math.tan((fovY / 2))
            tanHalfFovX = math.tan((fovX / 2))

            top = tanHalfFovY * znear
            bottom = -top
            right = tanHalfFovX * znear
            left = -right

            P = torch.zeros(4, 4)

            z_sign = 1.0

            P[0, 0] = 2.0 * znear / (right - left)
            P[1, 1] = 2.0 * znear / (top - bottom)
            P[0, 2] = (right + left) / (right - left)
            P[1, 2] = (top + bottom) / (top - bottom)
            P[3, 2] = z_sign
            P[2, 2] = z_sign * zfar / (zfar - znear)
            P[2, 3] = -(zfar * znear) / (zfar - znear)
            return P
        fovx = 2 * np.arctan(self.get_tanfov(w, fx))
        fovy = 2 * np.arctan(self.get_tanfov(h, fy))
        proj_mat = getProjectionMatrix(zNear, zFar, fovx, fovy).transpose(0, 1).to(self.device)
        full_mat = (self.get_viewmatrix(rotation, position).unsqueeze(0).bmm(proj_mat.unsqueeze(0))).squeeze(0)
        return full_mat



class BatchRasterizer(nn.Module):
    def __init__(self, gaussain_path, sh_degree, device="cuda"):
        super().__init__()
        self.device = device
        self.active_sh_degree = sh_degree
        self.load_ply(gaussain_path, sh_degree)
        

    def load_ply(self, path, sh_degree=3):
        plydata = PlyData.read(path)
        xyz = np.stack((np.asarray(plydata.elements[0]["x"]),
                        np.asarray(plydata.elements[0]["y"]),
                        np.asarray(plydata.elements[0]["z"])),  axis=1)
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]

        features_dc = np.zeros((xyz.shape[0], 3, 1))
        features_dc[:, 0, 0] = np.asarray(plydata.elements[0]["f_dc_0"])
        features_dc[:, 1, 0] = np.asarray(plydata.elements[0]["f_dc_1"])
        features_dc[:, 2, 0] = np.asarray(plydata.elements[0]["f_dc_2"])

        extra_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_rest_")]
        extra_f_names = sorted(extra_f_names, key = lambda x: int(x.split('_')[-1]))
        assert len(extra_f_names)==3*(sh_degree + 1) ** 2 - 3
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (sh_degree + 1) ** 2 - 1))

        scale_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("scale_")]
        scale_names = sorted(scale_names, key = lambda x: int(x.split('_')[-1]))
        scales = np.zeros((xyz.shape[0], len(scale_names)))
        for idx, attr_name in enumerate(scale_names):
            scales[:, idx] = np.asarray(plydata.elements[0][attr_name])

        rot_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("rot")]
        rot_names = sorted(rot_names, key = lambda x: int(x.split('_')[-1]))
        rots = np.zeros((xyz.shape[0], len(rot_names)))
        for idx, attr_name in enumerate(rot_names):
            rots[:, idx] = np.asarray(plydata.elements[0][attr_name])

        features_dc = torch.tensor(features_dc, dtype=torch.float, device=self.device).transpose(1, 2).contiguous()
        features_rest = torch.tensor(features_extra, dtype=torch.float, device=self.device).transpose(1, 2).contiguous()

        self.means3D = torch.tensor(xyz, dtype=torch.float, device=self.device)
        self.shs = torch.cat((features_dc, features_rest), dim=1).contiguous()
        self.opacities = torch.tensor(opacities, dtype=torch.float, device=self.device)
        self.scales = torch.tensor(scales, dtype=torch.float, device=self.device)
        self.rotations = torch.tensor(rots, dtype=torch.float, device=self.device)
        self.active_sh_degree = sh_degree

        self.scales = torch.exp(self.scales)
        self.opacities = torch.sigmoid(self.opacities)
        self.rotations = torch.nn.functional.normalize(self.rotations)
        print(f"Loaded {self.opacities.shape[0]} gaussians")

    def forward(self, camera, background):
        colors_precomp = torch.Tensor([])
        cov3Ds_precomp = torch.Tensor([])

        # torch.save({
        #     "raster_settings.bg":background, 
        #     "means3D" :self.means3D,
        #     "colors_precomp" :colors_precomp,
        #     "opacities" :self.opacities,
        #     "scales" :self.scales,
        #     "rotations" :self.rotations,
        #     "raster_settings.scale_modifier" :1,
        #     "cov3Ds_precomp" :cov3Ds_precomp,
        #     "raster_settings.viewmatrix" :camera.view_mat,
        #     "raster_settings.projmatrix" :camera.proj_mat,
        #     "raster_settings.tanfovx" :camera.tan_fovx,
        #     "raster_settings.tanfovy" :camera.tan_fovy,
        #     "raster_settings.image_height" :camera.height,
        #     "raster_settings.image_width" :camera.width,
        #     "sh" :self.shs,
        #     "raster_settings.sh_degree" :self.active_sh_degree,
        #     "raster_settings.campos" :camera.cam_pos,
        #     "raster_settings.prefiltered" :False,
        #     "raster_settings.debug" :False
        # }, r"D:\code\VanillaGS\output\test/vanilla_gs1.pth")
        

        # Invoke C++/CUDA rasterization routine
        rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer = batch_rasterize_gaussians(
            background,
            self.means3D,
            colors_precomp,
            self.opacities,
            self.scales, 
            self.rotations,
            1,
            cov3Ds_precomp,
            camera.view_mat,
            camera.proj_mat,
            camera.tan_fovx,
            camera.tan_fovy,
            camera.height, 
            camera.width,
            self.shs,
            self.active_sh_degree,
            camera.position,
            False,
            False)
        return rendered, out_color


def render_scene_batch(gaussian_path, camera_path, save_path=None, sh_degree=3, test_idx=None, batch_size=1):
    device = torch.device('cuda:0')
    bg_color = torch.zeros(3, dtype=torch.float32, device=device)  # black
    
    with open(camera_path, 'r') as camera_file:
        cameras_json = json.loads(camera_file.read())

    if save_path is not None:
        os.makedirs(save_path, exist_ok=True)
    
    rasterizer = BatchRasterizer(gaussian_path, sh_degree, device)

    for i in range(10):
        rasterizer.forward(BatchCamera([cameras_json[j] for j in range(batch_size)], device), bg_color)
    if test_idx is not None:
        if not isinstance(test_idx, list):
            test_idx = [test_idx + i for i in range(batch_size)]
        cameras_json = [cameras_json[i] for i in test_idx]

    times = []
    progress_bar = tqdm(cameras_json)
    for i in range(0, len(cameras_json), batch_size):
        camera = BatchCamera(cameras_json[i:min(i+batch_size, len(cameras_json))], device)

        torch.cuda.synchronize()
        t0 = time.time()
        num_rendered, images = rasterizer.forward(camera, bg_color)
        torch.cuda.synchronize()
        t1 = time.time()

        times.append((t1-t0)*1000)
        progress_bar.set_description(f"FPS = {1/(t1-t0) :2f}, time = {(t1-t0)*1000:.2f} ms, num_rendered = {num_rendered}")
        progress_bar.update(min(batch_size, len(cameras_json) - i))

        if save_path is not None:
            for j in range(min(batch_size, len(cameras_json) - i)):
                image_path = os.path.join(save_path, "%s.jpg" % camera.img_name[j].split('/')[-1])
                image = images[j].permute(1,2,0).cpu().numpy()
                # image = (image*255).astype(np.uint8)
                Image.fromarray(image).save(image_path.replace('.ppm', '.jpg'))
    progress_bar.close()
    print(f"Mean time cost = {np.mean(times) / batch_size:.2f} ms, Max time cost = {np.max(times):.2f} ms")
    plt.plot(times, label="Render time of VanillaGS")
    plt.xlabel("Frame")
    plt.ylabel("Time cost (ms)")
    plt.title("Render time of VanillaGS")
    plt.legend()
    plt.show()


if __name__ == "__main__":
    scenes = {
        "garden": {
            "gaussian_path": r"Z:/home/gwc/code/SuGaR/output/vanilla_gs/garden/point_cloud/iteration_7000/point_cloud.ply",
            "camera_path": r"Z:/home/gwc/code/SuGaR/output/vanilla_gs/garden/cameras.json",
            "save_path": r"output/garden/batch"
        },
        "mc_aerial_c36": {
            "gaussian_path": r"D:\data\mc_aerial_c36\point_cloud\iteration_30000\point_cloud.ply",
            "camera_path": r"D:\data\mc_aerial_c36\cameras.json",
            "save_path": r"output/mc_aerial_c36/batch"
        },
        "DP1258_dji_and_yy": {
            "gaussian_path": r"D:\data\DP1258_dji_and_yy_merge\point_cloud\iteration_7000\point_cloud.ply",
            "camera_path": r"D:\data\DP1258_dji_and_yy_merge\cameras.json",
            "save_path": r"output/DP1258_dji_and_yy/batch"
        },
        "DP1139": {
            "gaussian_path": r"D:\data\DP1139\high_res_merged\point_cloud\iteration_30000\point_cloud.ply",
            "camera_path": r"D:\data\DP1139\high_res_merged\cameras.json",
            "save_path": r"output/DP1139/batch"
        },
        "37009-47004": {
            "gaussian_path": r"D:\data\37009-47004\point_cloud\iteration_30000\point_cloud.ply",
            "camera_path": r"D:\data\37009-47004\cameras.json",
            "save_path": r"output/37009-47004/batch",
        },
    }

    scene = "garden"
    gaussian_path = scenes[scene]["gaussian_path"]
    camera_path = scenes[scene]["camera_path"]
    save_path = scenes[scene]["save_path"]
    render_scene_batch(gaussian_path, camera_path, save_path=None, sh_degree=3, test_idx=None, batch_size=1)